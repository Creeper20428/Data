//	VMachine BIOS
//	Write sectors (int 0x13/ah=0x03)
//
...
void FDWriteSectors(struct Registers * registers)
{
	...
	srcAddr = ((Dword)registers->r_es << 4) + registers->r_bx.w;

	//Does this read does span a 64k boundary?
	if(srcAddr >> 16 != (srcAddr + numBytes) >> 16)
	{
	...
	outportb(0x04, srcAddr & 0xff);
	outportb(0x04, (srcAddr >> 8) & 0xff);

	//Set the page
	outportb(0x81, (srcAddr >> 16) & 0xff);
	...
	*int0eReceived = 0x00;
	...
	outportb(0x3f5, 0x05);						//Non-multi-track, non-MFM, Write data command
...
void HDWriteSectors(struct Registers * registers)
{
	...
	*int76Received = 0x00;
	...
	for(sector = 0; sector < registers->r_ax.b.l; ++sector)
	{
		...
		//Await DRQ
		while((inportb(0x3f6) & 0x08) == 0)
			;

		//Write the data to the IDE controller
		for(i = 0; i < 0x20; ++i)
		{
			Word far * src = MK_FP(	(Word)(srcAddr >> 4) + sector * 0x20 + i,
									(Word)srcAddr & 0x0f);
			Byte j;
